...
plot(center(1) + rStart*cos(thetaPlot), center(2) + rStart*sin(thetaPlot), ...
    'w-', 'LineWidth', 1.2);
plot(center(1) + rEnd*cos(thetaPlot), center(2) + rEnd*sin(thetaPlot), ...
    'm-', 'LineWidth', 1.2);

% Waveguide region as two boundary arcs
...
legend({'center', sprintf('r = %.3f \\mum', rStart), sprintf('r = %.3f \\mum', rEnd)}, ...
...
text(0.60, 0.13, txt, 'Units', 'normalized', ...
    'BackgroundColor', 'w', 'EdgeColor', 'none', 'Interpreter', 'tex');

%% 4. Poling period vs radius
subplot(2,2,4);
hold on; grid on; box on;
yl = [min([ana.periodMean(:); lambdaDesign(:)], [], 'omitnan') - 0.15, ...
      max([ana.periodMean(:); lambdaDesign(:)], [], 'omitnan') + 0.15];
if any(~isfinite(yl)) || diff(yl) <= 0
    yl = [cfg.extract.minPeriodUm, cfg.extract.maxPeriodUm];
end
shade_radius_region(wgMin, wgMax, yl, cfg);

errorbar(rList, ana.periodMean, ana.periodStd, 'o-', 'LineWidth', 1.2, ...
    'MarkerSize', 3, 'CapSize', 4);
...
sgtitle(sprintf('%s | scan %d | %s | center shift = [%.3f, %.3f] \\mum', ...
    result.deviceName, result.scanID, result.rawFileName, ...
    result.centerShift(1), result.centerShift(2)), ...
    'FontWeight', 'bold', 'Interpreter', cfg.plot.interpreter);
